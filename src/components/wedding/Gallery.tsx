        ...
        <div className="relative mx-auto mt-12 max-w-5xl">
          ...
          <GalleryFrame>
            <div className="relative aspect-[4/5] overflow-hidden bg-[#17211f] sm:aspect-[16/10] md:aspect-[16/9]" onTouchStart={(event) => { touchStartX.current = event.touches[0]?.clientX ?? null; }} onTouchEnd={(event) => handleTouchEnd(event.changedTouches[0]?.clientX)}>
              <img key={current.src + index} src={current.src} alt={current.alt} loading="eager" decoding="async" className="gallery-cinematic-image absolute inset-0 h-full w-full object-cover" style={{ animation: direction === "right" ? "gallery-cinematic-in-right 900ms cubic-bezier(.2,.7,.2,1) both" : "gallery-cinematic-in-left 900ms cubic-bezier(.2,.7,.2,1) both" }} />
